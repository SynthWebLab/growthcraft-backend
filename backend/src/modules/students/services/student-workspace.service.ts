...
export class StudentWorkspaceService {
  private static instance: StudentWorkspaceService;

  private constructor() {}

  public static getInstance(): StudentWorkspaceService {
    if (!StudentWorkspaceService.instance) {
      StudentWorkspaceService.instance = new StudentWorkspaceService();
    }
    return StudentWorkspaceService.instance;
  }
    ...
    try {
      ...
      if (course?.instructorId) {
        ...
        if (profile && profile.userId) {
          ...
          resolvedInstructors.push({
            name: (course as any).instructor?.name || course.instructorId,
            designation: (course as any).instructor?.designation || 'Lead Course Instructor',
            avatar: (course as any).instructor?.avatar || '',
            meetingLink: 'https://meet.google.com/gc-course-session',
          });
        }
      }
...
export const studentWorkspaceService = StudentWorkspaceService.getInstance();
